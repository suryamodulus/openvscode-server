...
export async function setupServerServices(connectionToken: ServerConnectionToken, args: ServerParsedArgs, REMOTE_DATA_FOLDER: string, disposables: DisposableStore) {
	...
	if (supportsTelemetry(productService, environmentService)) {
		...
		if (productService.aiConfig && productService.aiConfig.segmentKey) {
			oneDsAppender = new GitpodInsightsAppender(productService.nameShort, productService.version, productService.aiConfig.segmentKey, productService.gitpodPreview, productService.extensionsGallery?.serviceUrl);
